...
public class CameraController : MonoBehaviour
{
    ...
    public float startRadius; // Small starting radius at the cone tip
    ...
    {
        if (other.CompareTag("Player")) Debug.Log("Player entered vision cone!");
    }

    private void OnTriggerExit(Collider other)
    {
        if (other.CompareTag("Player")) Debug.Log("Player exited vision cone!");
    }
    ...
    private void CreateConeMesh()
    {
        ...
        var vertices = new Vector3[segments + 2]; // +2 for tip and center of base
        var triangles = new int[segments * 3 * 2]; // Extra triangles for the base cap
        ...
        var baseCenter = startTransform.position + startTransform.forward * height;
        ...
        for (var i = 0; i < segments; i++)
        {
            ...
            var baseX = Mathf.Cos(angle) * baseRadius;
            var baseZ = Mathf.Sin(angle) * baseRadius;

            // Calculate the base vertices in world space
            var baseVertex = startTransform.position + new Vector3(baseX, 0, baseZ) + startTransform.forward * height;
            vertices[i + 1] = baseVertex;

            // 6. Define triangles for the cone's side
            var nextIndex = (i + 1) % segments + 1; // Next vertex index, wrapping around at the end
            ...
            var nextBaseIndex = (i + 1) % segments + 1;
